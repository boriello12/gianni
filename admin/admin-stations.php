...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        ...
        case 'edit_station':
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssddi", $street, $city, $municipality, $civic, $zipcode, $lat, $lng, $stationId);
            
            if ($stmt->execute()) {
                setFlashMessage('success', 'Station updated successfully.');
            } else {
                setFlashMessage('error', 'Error updating station.');
            }
            break;
...
// Get all stations
$stations = $conn->query("SELECT * FROM Stations ORDER BY station_id DESC")->fetch_all(MYSQLI_ASSOC);
